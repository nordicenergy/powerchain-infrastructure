...
function readInputs(){   
    ...
    if [ $ethNetwork == "ropsten" ]; then
      ...
      echo 'CONTRACT_ADDRESS=0x66586d8a9F1dd68D254a3E5c0222c25931EeBAe9' >> ./setup.conf
    elif [ $ethNetwork == "mainnet" ]; then
      echo 'INFURA_URL=wss://mainnet.infura.io/ws' >> ./setup.conf
      echo 'CONTRACT_ADDRESS=0x4D0A4C762BD7f742096DAAF5911dcf9C94b9ea95' >> ./setup.conf
    else 
...
function main(){
    ...
    if [ ! -f setup.conf ]; then
        ...
        echo -e '\e[1;32mFor accessing PowerChain UI, please open the following from a web browser \e[0m'$uiUrl
        echo -e '\e[1;32mTo join this node from a different host, please run PowerChain and choose option to run Join Network\e[0m'
